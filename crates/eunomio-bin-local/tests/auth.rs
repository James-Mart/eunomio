...
async fn count_auth_events(state: &eunomio_server::state::AppState, event_type: &str) -> i64 {
    test_db::query_i64(
        state,
...
async fn count_auth_sessions(state: &eunomio_server::state::AppState) -> i64 {
    test_db::query_i64_no_params(state, "SELECT COUNT(*) FROM auth_sessions").await
}
