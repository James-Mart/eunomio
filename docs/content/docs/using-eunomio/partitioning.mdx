...
```mermaid
flowchart BT
    Run[/Subagent runs/] --> Gate{HITL gate? default OFF}
...
All gates default to **off**. Turn any on in partition settings when you want to review that phase before the Coordinator advances. With `afterIndivisible` off, indivisible partitions finish automatically.
