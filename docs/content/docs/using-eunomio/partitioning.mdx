...
Each subagent follows the same phase pattern. The diagram below shows one cycle; it repeats for Surveyor, Planner, and Constructor, each with its own HITL gate (`afterSurvey`, `afterPlanning`, `afterConstruct`). Indivisible Planner outputs have their own confirmation gate (`afterIndivisible`).
