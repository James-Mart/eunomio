...
1. **Baseline** — cwd must be `{{WORKTREE_PATH}}`. Run plain `git` (no
   `-C` elsewhere). If `git rev-parse HEAD` ≠ `{{PARENT_COMMIT}}`, `cd
   {{WORKTREE_PATH}}` and retry — wrong cwd is not a baseline mismatch.
   ```bash
   git rev-parse HEAD          # → {{PARENT_COMMIT}}
   git rev-parse HEAD^{tree}   # → {{BEFORE_TREE}}
   ```
   Output `BLOCKED: worktree baseline mismatch` only when the first line
   matches and the second does not.
2. Read the full diff with
   ...
   in. `{{BEFORE_TREE}}` and `{{TARGET_TREE}}` are tree SHAs — use them
   directly in diff/show, not as commits.
...
- Baseline check fails per step 1.
